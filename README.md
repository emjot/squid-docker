...
- `SQUID_MAJOR_VERSION-alpineALPINE_MAJOR_AND_MINOR` (e.g. `7-alpine3.23`)
...
At the moment, we only build for a single alpine version (3.23 as of July 2026). That means we switch to newer alpine versions from time to time and discontinue building/updating images for the older version. If there is a need to support multiple alpine versions, get in touch by creating an issue (and possibly be prepared to create a PR ;-).
